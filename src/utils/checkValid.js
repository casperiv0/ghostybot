...
function checkValid() {
    ...
    if (config.ownerId === "") {
        console.warn(chalk.yellow("[WARNING]: ownerId is required for bot-owner commands"));
    }

    if (config.reportsChannelId === "") {
        console.warn(chalk.yellow("[WARNING]: reportsChannelId is required for the bugreport command"));
    }
}
