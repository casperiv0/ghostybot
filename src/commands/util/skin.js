...
module.exports = {
  ...
  if !args) return message.channel.send('Please write the name of the skin')

    MojangAPI.nameToUuid(args, (err, res) => {
    if (err)
        console.log(err);
    else {
        const uuid = res[0]?.id
        if(!uuid) return message.channel.send(`Player \`${args}\` not found!`)
